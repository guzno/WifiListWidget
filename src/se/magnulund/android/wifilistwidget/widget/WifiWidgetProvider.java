	...
	private void setWidgetActive(Context context, Boolean active) {
		...
		if (active) {
			...
			if (preferences.contains(Preferences.DEVICE_HAS_MOBILE_NETWORK) == false) {
				...
				editor.putBoolean(Preferences.DEVICE_HAS_MOBILE_NETWORK,
						MainActivity.deviceHasMobileNetwork(context));
			}
			if (preferences.contains(Preferences.WALLED_GARDEN_CHECK_DONE) == false) {
				Intent intent = new Intent(context,
						ConnectivityChangeService.class);
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (action.equals(CLICK_ACTION)) {
			...
			case ITEM_CLICK_CONNECT: {
				final int networkId = intent.getIntExtra(
						WifiWidgetProvider.NETWORK_ID, -1);
				if (networkId > -1) {
					...
					// AlarmUtility.ALARM_TYPE_BACKOFF);
				}
			...
			wifiManager.startScan();
			AlarmUtility.scheduleAlarm(context,
					AlarmUtility.ALARM_TYPE_SCAN_DELAY);
		}
	...
	public static void updateWidgets(Context context, int updateType,
		...
		if (appWidgetIds.length > 0) {
			...
			for (int appWidgetId : appWidgetIds) {
				WidgetRemoteViews widgetRemoteViews = new WidgetRemoteViews(
						context, appWidgetId);
			...
			case UPDATE_WIFI_STATE_CHANGED: {
				if (updateInfo == WifiManager.WIFI_STATE_DISABLING) {
					AlarmUtility.scheduleWifiStateChecker(context, updateInfo,
							1);
				}
				break;
			}
