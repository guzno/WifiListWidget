...
public class ConnectivityChangeService extends IntentService {
	...
	public static void isWalledGardenConnection(Context context,
		...
		boolean walledGardenChecked = preferences.getBoolean(
				Preferences.WALLED_GARDEN_CHECK_DONE, false);
		boolean isWalledGarden = preferences.getBoolean(
				Preferences.WALLED_GARDEN_CONNECTION, false);
		...
		if (walledGardenChecked == false || checkIfLoggedIn) {
			...
			try {
				...
				urlConnection
						.setConnectTimeout(WALLED_GARDEN_SOCKET_TIMEOUT_MS);
					...
					if (walledGarden) {
						...
						if (redirectURL == null) {
							...
							redirectURL = redirectURL.replace(
									WALLED_GARDEN_URL, LANDING_PAGE_URL);
							Log.e(TAG, "walled in to: " + redirectURL);
						}
					}
					urlConnection.disconnect();
					editor.putBoolean(Preferences.WALLED_GARDEN_CONNECTION,
							walledGarden);
					editor.putString(Preferences.WALLED_GARDEN_REDIRECT_URL,
							redirectURL);
					editor.commit();
				}
			}
