...
/**
 * Created with IntelliJ IDEA. User: erikeelde Date: 5/12/2012 Time: 24:15 To
...
public class AlarmReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent receivedIntent) {
		int alarmType = receivedIntent.getIntExtra(
				AlarmUtility.ALARM_IDENTIFIER, -1);
		...
		case AlarmUtility.ALARM_TYPE_SCAN_DELAY: {
			...
			updateInfo = receivedIntent.getIntExtra(AlarmUtility.UPDATE_INFO,
					-1);
		...
		case AlarmUtility.ALARM_TYPE_WIFI_STATE: {
			...
			if (wifiManager.getWifiState() == receivedIntent.getIntExtra(
					AlarmUtility.LAST_WIFI_STATE, -1)) {
				updateWidgets = false;
				restartAlarm = true;
			}
			break;
		...
		try {
			if (updateWidgets) {
				WifiWidgetProvider.updateWidgets(context, updateType,
						updateInfo);
			}
			...
			if (restartAlarm) {
				switch (alarmType) {
				case AlarmUtility.ALARM_TYPE_BACKOFF: {
					int attempt = receivedIntent.getIntExtra(
							AlarmUtility.IDENTIFIER_ALARM_ATTEMPT, -1);
					AlarmUtility.scheduleAlarmWithBackoff(context, ++attempt);
					break;
				}
				case AlarmUtility.ALARM_TYPE_SCAN_DELAY: {
					AlarmUtility.scheduleScanDelayAlarm(context,
							AlarmUtility.RE_ENABLE_SCANNING);
					break;
				}
				case AlarmUtility.ALARM_TYPE_WIFI_STATE: {
					if (restartAlarm) {
						int attempt = receivedIntent.getIntExtra(
								AlarmUtility.IDENTIFIER_ALARM_ATTEMPT, -1);
						AlarmUtility.scheduleWifiStateChecker(context,
